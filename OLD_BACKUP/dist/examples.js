...
async function exampleBasicUsage() {
    ...
    const server = (0, mcp_server_1.createGodotMCPServer)(config);
    try {
        // Get Godot version
...
async function exampleLaunchEditor() {
    ...
    const server = (0, mcp_server_1.createGodotMCPServer)({
        projectsPath: './client',
        debugMode: true
    });
...
async function exampleProjectStructure() {
    ...
    try {
        ...
        structure.scenes.forEach((scene) => console.log(`    - ${scene}`));
        console.log(`  Scripts (${structure.scripts.length}):`);
        structure.scripts.forEach((script) => console.log(`    - ${script}`));

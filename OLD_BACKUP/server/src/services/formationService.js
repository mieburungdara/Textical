...
class FormationService {
    async updateFormation(userId, presetId, slots) {
        ...
        if (listedHeroes > 0) throw new AppError(ErrorCodes.HERO_LISTED_MARKET, 'Cannot add a hero to formation while they are listed in the market.');
    ...
    async moveUnit(userId, presetId, heroId, x, y) {
        ...
        if (!hero) throw new AppError(ErrorCodes.HERO_UNAUTHORIZED, 'Unauthorized.');
    ...
    async swapUnits(userId, presetId, heroA, heroB) {
        ...
        if (owned !== 2) throw new AppError(ErrorCodes.HERO_UNAUTHORIZED, 'Unauthorized.');

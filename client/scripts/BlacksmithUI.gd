...
func _close_other_panels() -> void:
    ...
    if game_scene:
        ...
        var shop_ui = game_scene.get_node_or_null("UILayer/OverlayContainer/ShopUI")
...
func _create_equipment_slot(slot_name: String, slot_label: String, item: Dictionary) -> Control:
    ...
    container.custom_minimum_size = Vector2(0, GameTheme.SIZE_BUTTON_LARGE + GameTheme.SPACING_SMALL)
    
    if item.is_empty():
        container.setup(slot_label + " [Empty]", "", GameTheme.COLOR_TEXT_DISABLED)

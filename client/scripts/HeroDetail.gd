...
func _refresh_info() -> void:
    ...
    # Traits
    var traits_list: Array = hero_data.get("traits", [])
    if traits_list.size() > 0:
        _add_section_title(info_container, "🎯 TRAITS")
        var i: int = 0
        while i < traits_list.size():
            _add_info_row(info_container, "•", str(traits_list[i]))
            i += 1
...
func _add_section_title(container: VBoxContainer, title: String) -> void:
    ...
    label.modulate = GameTheme.COLOR_ACCENT
...
func _add_stat_row(container: VBoxContainer, stat_name: String, value: int, description: String) -> void:
    ...
    row.custom_minimum_size = Vector2(0, GameTheme.SPACING_LARGE + 8)
    
    var name_label = Label.new()
    name_label.text = stat_name
    name_label.custom_minimum_size = Vector2(GameTheme.SPACING_LARGE * 2 + GameTheme.SPACING_SMALL, 0)
    name_label.modulate = GameTheme.COLOR_TEXT_PRIMARY
    row.add_child(name_label)
    
    var value_label = Label.new()
    value_label.text = str(value)
    value_label.custom_minimum_size = Vector2(GameTheme.SPACING_LARGE + GameTheme.SPACING_SMALL, 0)
    value_label.modulate = GameTheme.COLOR_ACCENT
    value_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
    row.add_child(value_label)
    
    var desc_label = Label.new()
    desc_label.text = " - " + description
    desc_label.modulate = GameTheme.COLOR_TEXT_SECONDARY
...
func _add_equip_slot(container: VBoxContainer, slot_name: String, item_name: String, slot_type: EquipSlotType = EquipSlotType.WEAPON) -> void:
    ...
    slot_label.custom_minimum_size = Vector2(GameTheme.SPACING_LARGE * 4, 0)
    slot_label.modulate = GameTheme.COLOR_TEXT_SECONDARY
    row.add_child(slot_label)
    
    # Make item clickable using UIButton component
    var item_button = UIButton.new()
    item_button.setup(item_name)
    item_button.custom_minimum_size = Vector2(GameTheme.SPACING_LARGE * 6, 0)
    ...
    if item_name != "[Empty]" and item_name != "None":
        ...
        unequip_btn.setup("✕")
        unequip_btn.custom_minimum_size = Vector2(GameTheme.SIZE_BUTTON_SMALL, 0)
...
func _add_skill_row(container: VBoxContainer, skill: Dictionary) -> void:
    ...
    var icon = Label.new()
    icon.text = "•"
    icon.custom_minimum_size = Vector2(GameTheme.SPACING_MEDIUM, 0)
    icon.modulate = GameTheme.COLOR_PRIMARY
    row.add_child(icon)
    ...
    level_label.modulate = GameTheme.COLOR_SUCCESS
...
func _add_info_row(container: VBoxContainer, label_text: String, value_text: String) -> void:
    var row = HBoxContainer.new()
    row.custom_minimum_size = Vector2(0, GameTheme.SPACING_LARGE + 6)
    
    var label = Label.new()
    label.text = label_text + ":"
    label.custom_minimum_size = Vector2(GameTheme.SPACING_LARGE * 4, 0)
    label.modulate = GameTheme.COLOR_TEXT_SECONDARY
    row.add_child(label)
    
    var value = Label.new()
    value.text = value_text
    value.modulate = GameTheme.COLOR_TEXT_PRIMARY

...
const COLOR_PLAYER := GameTheme.COLOR_PRIMARY
const COLOR_SELECTED := GameTheme.COLOR_ACCENT
const COLOR_HOVER := GameTheme.COLOR_SUCCESS
const COLOR_EMPTY := GameTheme.COLOR_SURFACE_LIGHT
...
func _create_grid_cell(index: int, pos: Vector2i, heroes: Array) -> Control:
    ...
    container.custom_minimum_size = Vector2(GameTheme.SIZE_GRID_MEDIUM, GameTheme.SIZE_GRID_MEDIUM - 10)
    ...
    pos_label.add_theme_font_size_override("font_size", GameTheme.FONT_CAPTION)
    ...
    if index < heroes.size():
        ...
        empty_panel.custom_minimum_size = Vector2(GameTheme.SIZE_ICON_MEDIUM, GameTheme.SIZE_ICON_MEDIUM + 5)
        var empty_style = StyleBoxFlat.new()
        empty_style.bg_color = COLOR_EMPTY
        empty_style.set_corner_radius_all(GameTheme.RADIUS_SMALL)
        ...
        empty_label.modulate = GameTheme.COLOR_TEXT_DISABLED
...
func _create_hero_panel(hero: Dictionary, index: int) -> UICard:
    ...
    var card = UICard.new()
    card.custom_minimum_size = Vector2(GameTheme.SIZE_ICON_MEDIUM, GameTheme.SIZE_ICON_MEDIUM + 5)
    card.setup({
        "title": hero_name,
        "subtitle": hero_class,
        "content": "Lv.%d" % level,
        ...
        if style:
            if highlight:
                if is_empty:
                    ...
                    style.border_color = GameTheme.COLOR_SUCCESS
                else:
                    # Yellow for swap
                    style.border_color = COLOR_HOVER
                style.set_border_width_all(GameTheme.BORDER_MEDIUM)
            else:
                style.set_border_width_all(GameTheme.BORDER_NONE)
...
func _show_hero_details(hero: Dictionary) -> void:
    ...
    name_label.add_theme_font_size_override("font_size", GameTheme.FONT_SUBTITLE)
    name_label.modulate = GameTheme.COLOR_ACCENT
    detail_content.add_child(name_label)
    
    # Hero Class
    var class_label = Label.new()
    class_label.text = "Class: " + hero.get("class", "Novice")
    class_label.add_theme_font_size_override("font_size", GameTheme.FONT_BODY)
    detail_content.add_child(class_label)
    
    # Hero Level
    var level_label = Label.new()
    level_label.text = "Level: %d" % hero.get("level", 1)
    level_label.add_theme_font_size_override("font_size", GameTheme.FONT_BODY)
    detail_content.add_child(level_label)
    
    # Status
    var status_label = Label.new()
    status_label.text = "Status: " + hero.get("status", "active").to_upper()
    status_label.add_theme_font_size_override("font_size", GameTheme.FONT_BODY)
    var status = hero.get("status", "active")
    match status:
        "active":
            status_label.modulate = GameTheme.COLOR_SUCCESS
        "resting":
            status_label.modulate = GameTheme.COLOR_WARNING
        "dead":
            status_label.modulate = GameTheme.COLOR_DANGER
    detail_content.add_child(status_label)
    
    # Hero ID
    var id_label = Label.new()
    id_label.text = "ID: " + hero.get("id", "unknown")
    id_label.add_theme_font_size_override("font_size", GameTheme.FONT_CAPTION)
    id_label.modulate = GameTheme.COLOR_TEXT_SECONDARY
...
func _on_view_pressed() -> void:
    ...
    var game_scene = get_tree().root.get_node_or_null("GameScene")
    if game_scene:
        var hero_detail = game_scene.get_node_or_null("UILayer/OverlayContainer/HeroDetail")

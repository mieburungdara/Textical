...
func _create_item_slot(index: int, item: Dictionary) -> Control:
	...
	container.custom_minimum_size = Vector2(Theme.SIZE_ICON_LARGE, Theme.SIZE_ICON_LARGE)
	container.set_meta("item_data", item)
	container.mouse_filter = Control.MOUSE_FILTER_STOP
	
	# Background based on rarity using UIGridSlot style
	var rarity_color = Theme.get_rarity_color(item.get("rarity", "common"))
	var style = StyleBoxFlat.new()
	style.bg_color = Theme.darken(rarity_color, 0.5)

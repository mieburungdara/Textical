...
static func create_panel_style(bg_color: Color, radius: int = RADIUS_MEDIUM, border_color: Color = Color.NONE, border_width: int = 0) -> StyleBoxFlat:
	var style = StyleBoxFlat.new()
	style.bg_color = bg_color
	style.set_corner_radius_all(radius)
	
	if border_color != Color.NONE and border_width > 0:

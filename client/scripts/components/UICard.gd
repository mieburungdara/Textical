...
func _build_default_card() -> void:
	...
	style.bg_color = Theme.COLOR_SURFACE
	style.set_corner_radius_all(Theme.RADIUS_MEDIUM)
	style.border_color = Theme.COLOR_SURFACE_LIGHT
	style.set_border_width_all(Theme.BORDER_THIN)
	add_theme_stylebox_override("panel", style)
	
	# Default size
	custom_minimum_size = Vector2(Theme.SIZE_MEDIUM_WIDTH, Theme.SIZE_ICON_LARGE * 2)
	
	# VBox container
	vbox = VBoxContainer.new()
	vbox.add_theme_constant_override("separation", Theme.SPACING_SMALL)
	add_child(vbox)
	...
	# Title
	title_lbl = Label.new()
	title_lbl.add_theme_font_size_override("font_size", Theme.FONT_BODY)
	...
	subtitle_lbl.add_theme_font_size_override("font_size", Theme.FONT_CAPTION)
	subtitle_lbl.modulate = Theme.COLOR_TEXT_SECONDARY
	vbox.add_child(subtitle_lbl)
	
	# Content
	content_lbl = Label.new()
	content_lbl.add_theme_font_size_override("font_size", Theme.FONT_CAPTION)
	content_lbl.modulate = Theme.COLOR_TEXT_SECONDARY

...
func _setup_widget() -> void:
	add_theme_constant_override("separation", Theme.SPACING_TINY)
	...
	notifications_container.add_theme_constant_override("separation", Theme.SPACING_TINY)
...
func _create_notification_item(notif: Dictionary, index: int) -> Control:
	...
	style.bg_color = Theme.darken(bg_color, 0.5)
	...
	hbox.add_theme_constant_override("separation", Theme.SPACING_SMALL)
	container.add_child(hbox)
	
	# Icon
	var icon = Label.new()
	icon.text = notif.get("icon", "ℹ️")
	icon.add_theme_font_size_override("font_size", Theme.FONT_BODY)
	hbox.add_child(icon)
	
	# Message
	var message = Label.new()
	message.text = notif.get("message", "")
	message.add_theme_font_size_override("font_size", Theme.FONT_CAPTION)
	message.modulate = Theme.COLOR_TEXT_PRIMARY
...
func _get_type_color(notif_type: String) -> Color:
	match notif_type:
		"success": return Theme.COLOR_SUCCESS
		"warning": return Theme.COLOR_WARNING
		"error": return Theme.COLOR_DANGER
		"quest": return Theme.COLOR_ACCENT
		"combat": return Theme.COLOR_PRIMARY
		_: return Theme.COLOR_SECONDARY

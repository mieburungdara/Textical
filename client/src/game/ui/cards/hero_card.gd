...
func setup(data: UnitData):
	...
	var r_color = Color.WHITE
	if data is HeroData:
		r_color = data.get_rarity_color()
		name_label.add_theme_color_override("font_color", r_color)
		class_label.text = data.current_job.display_name if data.current_job else "NO CLASS"
		level_label.text = "Lv. %d" % data.level
		
		# Update Stats Display (Total = Base + Bonus)
		hp_val.text = str(data.hp_base + data.hp_bonus)
		atk_val.text = str(data.damage_base + data.damage_bonus)
		def_val.text = str(data.defense_base) # def bonus not yet implemented
	else:
	...
	style.border_color = r_color.lerp(Color.BLACK, 0.3)

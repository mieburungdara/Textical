...
@onready var nav_hbox = $DockPanel/Margin/NavHBox if has_node("DockPanel/Margin/NavHBox") else null
@onready var hub_btn = $DockPanel/Margin/NavHBox/Town if has_node("DockPanel/Margin/NavHBox/Town") else null

# === AP DISPLAY NODE REFERENCES ===
@onready var ap_display = $DockPanel/Margin/APDisplay if has_node("DockPanel/Margin/APDisplay") else null
@onready var ap_current_label = $DockPanel/Margin/APDisplay/CurrentLabel if has_node("DockPanel/Margin/APDisplay/CurrentLabel") else null
@onready var ap_max_label = $DockPanel/Margin/APDisplay/MaxLabel if has_node("DockPanel/Margin/APDisplay/MaxLabel") else null
@onready var ap_bar = $DockPanel/Margin/APDisplay/APBar if has_node("DockPanel/Margin/APDisplay/APBar") else null

# === REGEN DISPLAY NODE REFERENCES ===
@onready var regen_display = $DockPanel/Margin/RegenDisplay if has_node("DockPanel/Margin/RegenDisplay") else null
@onready var hp_regen_label = $DockPanel/Margin/RegenDisplay/HPRegenLabel if has_node("DockPanel/Margin/RegenDisplay/HPRegenLabel") else null
@onready var mp_regen_label = $DockPanel/Margin/RegenDisplay/MPRegenLabel if has_node("DockPanel/Margin/RegenDisplay/MPRegenLabel") else null
...
func _ready():
    ...
    # Connect fixed routes only if nav_hbox exists
    if nav_hbox:
        for btn_name in ROUTES.keys():
            var btn = nav_hbox.get_node(btn_name) if nav_hbox.has_node(btn_name) else null
            if btn:
...
func _update_ui():
    ...
    var all_btns = {}
    if hub_btn:
        all_btns["Town"] = hub_btn
    if nav_hbox:
        for k in ROUTES.keys(): 
            all_btns[k] = nav_hbox.get_node(k) if nav_hbox.has_node(k) else null

...
@onready var wilderness_btn = $Actions/WildernessButton
...
func _ready():
	...
	wilderness_btn.pressed.connect(_on_wilderness_pressed)
...
func _on_wilderness_pressed():
	ServerConnector.travel(GameState.current_user.id, 2)
	wilderness_btn.text = "Traveling... (15s)"
	wilderness_btn.disabled = true
...
func _on_request_completed(endpoint, data):
	if "tavern/enter" in endpoint:
		...
		GameState.current_user.currentRegion = 2

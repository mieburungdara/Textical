...
func _start_cinematic_travel(task):
	# DEBUG: Log the task payload
	print("[ATLAS_DEBUG] Travel Payload: ", task)
	...
	var target_val = task.get("targetRegionId", 1)
	var target_rid = int(str(target_val).to_float())
	...
	print("[ATLAS_DEBUG] Start Pos: ", start_pos, " | End Pos: ", end_pos)
	
	if start_pos == end_pos:
		print("[ATLAS_WARNING] Start and End positions are identical. Animation will be invisible.")

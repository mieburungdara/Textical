...
func _on_request_completed(endpoint, data):
	if endpoint.contains("/regions") and data is Array:
		...
		status_label.text = "Traveling to destination..."
		# Let the TaskHUD handle the timer
		# The transition will happen after the timer ends
		_wait_for_travel_completion(data.targetRegionId)
	elif endpoint.contains("/region/"):
		# Update global state and move scene
...
func _wait_for_travel_completion(target_id):
	await get_tree().create_timer(15.0).timeout
	ServerConnector.get_region_details(target_id)
...
func _draw_map():
	...
	for region in regions:
		...
		if region.id == current_region_id:
			...
			travel_btn.text = "Here"
		elif not is_connected:
			travel_btn.disabled = true
			travel_btn.text = "Unreachable"

...
func _ready():
    _setup_ui()
    _connect_signals()
    print("[HeroGridContainer] _ready called, loading heroes...")
    _load_heroes()

func _setup_ui():
    # Setup grid columns (more columns for portrait mode)
    grid_container.columns = 4
    grid_container.custom_minimum_size = Vector2(0, 0)
...
func _connect_signals():
    ...
    if GameState:
        # Listen for heroes loaded from server
        if GameState.has_signal("heroes_loaded"):
            GameState.heroes_loaded.connect(_on_heroes_loaded)
            print("[HeroGridContainer] Connected to GameState.heroes_loaded signal")
        
        # Listen for heroes loading failed
        if GameState.has_signal("heroes_loading_failed"):
            GameState.heroes_loading_failed.connect(_on_heroes_loading_failed)
            print("[HeroGridContainer] Connected to GameState.heroes_loading_failed signal")

func _on_heroes_loaded(count: int):
    print("[HeroGridContainer] Heroes loaded from server: ", count)
    _load_heroes()

func _on_heroes_loading_failed(error: String):
    print("[HeroGridContainer] Heroes loading failed: ", error)
...
func _load_heroes():
    ...
    # Check if heroes are already loaded
    var heroes = GameState.current_heroes if GameState else []
    
    # If no heroes loaded yet, fetch from server
...
func _continue_load_heroes(heroes: Array):
    print("[HeroGridContainer] Found ", heroes.size(), " heroes in GameState")
    ...
    if heroes.is_empty():
        print("[HeroGridContainer] No heroes found, showing empty state")
        _show_empty_state()
        return
    
    # Create hero cards
    for hero in heroes:
        print("[HeroGridContainer] Creating card for hero: ", hero.get("name", "Unknown"))
...
func _create_hero_card(hero_data: Dictionary):
    # Load and instantiate HeroCard.tscn instead of creating Button + script
    var hero_card_scene = load("res://src/ui/components/HeroCard.tscn")
    var card = hero_card_scene.instantiate()
    
    # Set hero data (after instantiation so nodes exist)

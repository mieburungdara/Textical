...
Sistem ini mendukung pertarungan dalam berbagai skala, mulai dari pertempuran satu hero melawan satu monster hingga pertempuran masif dengan hingga **2.500 unit** secara bersamaan di atas grid 50x50. Arsitektur modular memungkinkan pengembangan dan pemeliharaan yang efisien, dengan pemisahan yang jelas antara komponen Grid, AI, Rules, dan Logger.
...
**Grid System** mengelola posisi dan pergerakan unit di atas medan perang. Grid yang digunakan memiliki dimensi **50x50**, yang memungkinkan hingga 2.500 unit berpartisipasi dalam satu pertempuran. Setiap sel dalam grid dapat berisi maksimal satu unit, dan pergerakan unit dibatasi oleh rules tertentu seperti biaya movement berdasarkan terrain dan obstacle yang ada di medan.
...
### 2.3 Mekanisme Pemrosesan Tick

Sistem combat Textical menggunakan mekanisme **Simultaneous Tick** untuk memproses aksi. Dalam satu tick simulasi (biasanya setara dengan satu detik dalam waktu game), semua unit yang memiliki action points yang cukup akan melakukan aksi secara bersamaan. Pendekatan ini menghindari masalah "first mover advantage" yang dapat terjadi dalam sistem berbasis turn-based murni.
...
Perhitungan damage dalam sistem Textical mengikuti formula dasar berikut:

```
Damage Final = (Base Damage × Attack Multiplier × Elemental Multiplier × Trait Multiplier)
               × Critical Multiplier × Defense Reduction × Random Variance
```

Setiap komponen dalam formula ini memiliki fungsi spesifik dan dapat dimodifikasi oleh berbagai faktor dalam sistem. Berikut adalah penjelasan detail untuk setiap komponen:

**Base Damage** adalah nilai damage dasar yang berasal dari statistik attack unit dan damage yang ditentukan oleh skill atau ability yang digunakan. Base damage dihitung menggunakan statistik attack unit setelah melalui pipeline 12-layer stat calculation, memastikan bahwa semua modifier dari equipment, buffs, dan otros faktor sudah diperhitungkan.

**Attack Multiplier** adalah multiplier yang diterapkan berdasarkan kondisi serangan, seperti bonus damage dari posisi (high ground memberikan +15%), bonus dari formasi, atau bonus dari synergy aktif. Multiplier ini dapat berupa nilai flat atau persentase yang dikalikan dengan base damage.

**Elemental Multiplier** ditentukan oleh interaksi antara elemen penyerang dan elemen target. Sistem elemen Textical memiliki hubungan rock-paper-scissors antar elemen, dengan multiplier 1.5x untuk serangan kuat dan 0.5x untuk serangan lemah. Affinitas elemental dari kedua belah pihak juga mempengaruhi multiplier ini.

**Trait Multiplier** diterapkan berdasarkan trait yang dimiliki oleh unit penyerang atau target. Beberapa trait seperti "Berserker" memberikan bonus damage dalam kondisi tertentu, sementara trait lain seperti "Glass Cannon" memberikan bonus damage yang signifikan dengan trade-off pada defense.

**Critical Multiplier** diterapkan ketika serangan menghasilkan critical hit. Base critical multiplier adalah 2.0x (200%), tetapi dapat dimodifikasi oleh equipment, buffs, dan skill menjadi nilai yang lebih tinggi atau lebih rendah.

**Defense Reduction** menghitung seberapa besar damage yang dikurangi oleh defense target. Formula yang digunakan adalah:

```
Damage After Defense = Base Damage × (Defense / (Defense + Constant))
```

Dimana Constant adalah nilai tetap (biasanya 100) yang memastikan bahwa damage tidak pernah menjadi 0 atau negatif secara teori, tetapi dalam implementasi praktis, damage minimum adalah 1 untuk memastikan bahwa serangan selalu menghasilkan setidaknya sedikit damage.

**Random Variance** menambahkan variasi ke damage yang dihasilkan, dengan range ±10% dari nilai damage yang dihitung. Variasi ini dihitung sebagai:

```
Final Damage = Calculated Damage × (0.9 + (Random × 0.2))
```

Dimana Random adalah nilai antara 0 dan 1.
...
```
Distance = √((targetX - attackerX)² + (targetY - attackerY)²)
```
...
**Mana** adalah primary resource untuk magical abilities. Mana regenerasi secara pasif setiap tick (+5% max mana per tick) dan dapat direcharge melalui:
- MP potions atau items
- Skills dengan mana restore effects
- Trait "Thinker" yang memberikan +5 MP per turn start
...
### 9.1 Struktur File Combat System

```
server/
├── src/
│   ├── logic/
│   │   ├── combatEngine.js        # Main combat engine
│   │   ├── damageCalculator.js    # Damage calculation logic
│   │   ├── statusEffectSystem.js  # Status effects handling
│   │   ├── targetSelector.js      # Targeting logic
│   │   └── cooldownManager.js     # Cooldown management
│   ├── services/
│   │   ├── combatService.js       # Public API for combat
│   │   ├── statService.js         # EnhancedStat calculations
│   │   ├── gridService.js         # Grid management
│   │   └── aiService.js           # AI behavior execution
│   └── utils/
│       ├── combatLogger.js        # Battle logging
│       ├── elementalResolver.js   # Elemental interactions
│       └── comboDetector.js       # Combo system detection
└── tests/
    ├── combat/

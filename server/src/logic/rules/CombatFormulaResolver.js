...
class CombatFormulaResolver {
     ...
     */
    static calculateHitChance(attacker, defender, directionalBonus = 0) {
     ...
     */
    static calculateCriticalHit(attacker, defender, directionalBonus = 0) {
    ...
    static calculateBlockParry(defender, attacker, bypassBlock = false, sim = null) {
        ...
        if (rolled < finalParryChance) {
            ...
            if (sim) traitService.executeHook("onParry", defender, attacker, sim);
        } else if (rolled < finalBlockChance + finalParryChance) {
            result.blocked = true;
            result.damageMult = 1.0 - (blockPower + (mod.blockPowerMod || 0));
            if (sim) traitService.executeHook("onBlock", defender, attacker, sim);
        }
